...
class TestFormatChangePts:
    def test_negative_zero_displays_unsigned(self):
        assert metrics.format_change_pts(-0.004) == "0.00 pts"
        assert metrics.format_change_pts(-0.0) == "0.00 pts"
        assert metrics.format_change_pts(0.0) == "0.00 pts"
        assert metrics.format_change_pts(0.004) == "0.00 pts"

    def test_signed_values_keep_sign_two_decimals(self):
        assert metrics.format_change_pts(2.04) == "+2.04 pts"
        assert metrics.format_change_pts(-0.8) == "-0.80 pts"
        assert metrics.format_change_pts(-0.05) == "-0.05 pts"
        assert metrics.format_change_pts(0.15) == "+0.15 pts"
